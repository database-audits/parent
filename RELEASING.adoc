...
* Each repo to be released is on `main`, with no uncommitted *tracked* changes and `main` level
  with `origin/main`. Untracked/loose files are ignored. Releases must be cut from `main`:
  `release.yml` refuses to publish a tag whose commit is not on `origin/main`, so merge and push
